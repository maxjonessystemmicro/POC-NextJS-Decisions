...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      ...
      const url = new URL("https://decisions-systemmicro.ca/Primary/restapi/Flow/01JBAARCM2E3V9EZKDVCART0DS");
      url.searchParams.append('guest', 'true');
      url.searchParams.append('outputtype', 'Json');
      if (RoomID) {
        url.searchParams.append('RoomID', RoomID); // Add RoomID to the query parameters
      }
